...
const startMedSession = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user._id, userType: 2 }, process.env.JWT_SECRET);
        req.session.token = token;
        newTokendb(med, 2, token);
        return res.status(200).send(token);
      } catch (error) {
        console.log(error);
        return res.status(500).send('Internal server error');
...
const startPharmacianSession = async (req, res) => {
   try {
        ...
        const token = jwt.sign({ id: user._id, userType: 3 }, process.env.JWT_SECRET);
        req.session.token = token;
        newTokendb(pharmacian, 3, token);
        return res.status(200).send(token);
    } catch (error) {
        console.log(error);
        return res.status(500).send('Internal server error');
